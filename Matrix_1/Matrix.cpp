...
double matrix :: Determinant() const {
		{
			...
			if (m == n) {
				...
				case (2): {
					return(
						Get_Element(0, 0) * Get_Element(1, 1) -
						Get_Element(1, 0) * Get_Element(0, 1)
						); } break;
				case (3): {
					return (
						Get_Element(0, 0) * Get_Element(1, 1) * Get_Element(2, 2) +
						Get_Element(2, 0) * Get_Element(0, 1) * Get_Element(1, 2) +
						Get_Element(1, 0) * Get_Element(2, 1) * Get_Element(0, 2) -
						Get_Element(2, 0) * Get_Element(1, 1) * Get_Element(0, 2) -
						Get_Element(0, 0) * Get_Element(2, 1) * Get_Element(1, 2) -
						Get_Element(1, 0) * Get_Element(0, 1) * Get_Element(2, 2)
						); } break;
				case (4): {
					return (det4()); } break;
				default: {
		...
		for (i = 0, in = 0; i < size_minor + 1; i++) {
			if (i != row) {
				for (j = 0, jn = 0; j < size_minor + 1; j++) {
					if (j != col) {
						vv.at(in).at(jn++) = Get_Element(i, j);
					}
				}
				in++;
			}
...
double matrix :: det4() const {
		...
		m03 = {
			...
			{Get_Element(2,0),Get_Element(2,1),Get_Element(2,2)},
			{Get_Element(3,0),Get_Element(3,1),Get_Element(3,2)}
		};
		m13 = {
			{Get_Element(0,0),Get_Element(0,1),Get_Element(0,2)},
			{Get_Element(2,0),Get_Element(2,1),Get_Element(2,2)},
			{Get_Element(3,0),Get_Element(3,1),Get_Element(3,2)}
		};
		m23 = {
		...
		return(
			(-1) * Get_Element(0, 3) * d0
			+
			(+1) * Get_Element(1, 3) * d1
			+
			(-1) * Get_Element(2, 3) * d2
			+
			(+1) * Get_Element(3, 3) * d3
			);
	}
...
matrix operator*(const double& value, const matrix& M) {
	...
	for (size_t i = 0; i < m; i++)
		for (size_t j = 0; j < n; j++)
			vv.at(i).at(j) = M.Get_Element(i, j) * value;
		...
		for (size_t i = 0; i < m1; i++)
			for (size_t j = 0; j < n2; j++)
			{
				...
				for (size_t k = 0; k < n1; k++)
					vv.at(i).at(j) += lhs.Get_Element(i, k) * rhs.Get_Element(k, j);
			}
	...
	if (m1 == m2 && n1 == n2) {
		...
		for (size_t i = 0; i < m1; i++)
			for (size_t j = 0; j < n1; j++)
				vv.at(i).at(j) = (lhs.Get_Element(i, j) + rhs.Get_Element(i, j));
